...
export const cartSlice = createSlice({
  ...
  reducers: {
    addToCart: (state, action) => {
      ...
      state.products.forEach((cartProduct, index) => {
        if (cartProduct.id === newProduct.id) {
          console.log("check: ", newProduct.id);
          cartProduct.qty += 1;
          cartProduct.price += newProduct.price;
          state.products[index].qty = cartProduct.qty;
          state.products[index].price = cartProduct.price;
        }
      });
